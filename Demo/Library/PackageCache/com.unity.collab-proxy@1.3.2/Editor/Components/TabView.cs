...
namespace Unity.Cloud.Collaborate.Components
{
    ...
    internal class TabView : VisualElement
    {
        ...
        public TabView()
        {
            ...
            m_SavedTabIndex = WindowCache.Instance.TabIndex;
            ...
            // Save active tab index and ensure it's a valid value.
            WindowCache.Instance.BeforeSerialize += () => WindowCache.Instance.TabIndex = Math.Max(m_ActiveTabIndex, 0);
        }
        ...
        public void AddTab(string tabName, TabPageComponent tab)
        {
            ...
            var indexCopy = index;
            var btn = new TextButton(() => SwitchTab(indexCopy)) { text = tabName };
        ...
        /// <summary>
        /// Called once all required tabs are added and it will restore the active tab to what it was before the domain
        /// reload occured.
        /// </summary>
        public void Init()
        {
            if (m_SavedTabIndex == k_NoTabs) return;
            if (m_SavedTabIndex < m_TabList.Count)
            {
                SwitchTab(m_SavedTabIndex);
            }
            m_SavedTabIndex = k_NoTabs;
        }

        void SwitchTab(int index)
        {
            // Reset tab state of previously active content/button.
            if (m_ActiveTabIndex != k_NoTabs)
            {
